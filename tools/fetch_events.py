...
from sources import base, npb, bigsight, dome, ariake, zepp, garden_theater, nntt, kabukiza, national_stadium, medical_society, nougakudo, annual, forum, weather  # noqa: E402
...
def main():
    ...
    ap.add_argument("--sources", default="npb,bigsight,dome,ariake,zepp,garden_theater,nntt,kabukiza,national_stadium,medical_society,nougakudo,annual,forum", help="使うソース（カンマ区切り）")
    ...
    if "zepp" in wanted:
        try:
            got = zepp.fetch()
            print(f"[zepp] {len(got)}件（Zepp Haneda / Zepp DiverCity）")
